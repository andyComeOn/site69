...
final class CategoryModel extends BaseModel
{
	...
	public function fetchAllWithJoin()
	{
		...
		// SELECT category.*,count(article.id) AS article_count FROM category
		// SELECT category.*,count(article.id) AS article_count FROM category LEFT JOIN article ON category.id=article.category_id GROUP BY category.id 
	}
}
